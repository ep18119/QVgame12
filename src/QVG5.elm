    ...
    case msg of
        ...
        GetMessage command -> (
            case command of
                ...
                "SendProg" -> (model, sendMes (Encode.encode 2 (encodeASTRoots model)))
                "Delete" -> (model, sendMes "Reset")
                "Func0" -> ({model | nowTab = 0}, Cmd.none)
                "Func1" -> ({model | nowTab = 1}, Cmd.none)
                "Func2" -> ({model | nowTab = 2}, Cmd.none)
                "Func3" -> ({model | nowTab = 3}, Cmd.none)
                "Func4" -> ({model | nowTab = 4}, Cmd.none)
                "Func5" -> ({model | nowTab = 5}, Cmd.none)
                _ -> (addNewBrick command model, Cmd.none)
            )
        CheckReset ->
            (model, sendMes "Reset")
    ...
    in    
        if node.getBrickType == EntryBrick then
            ...
            in
                ...
                if isChanged == Changed || pickX < 98 then
...
rePos (ASTxy xy (ASTne node bottom right)) new model =
    if not model.getDnDInfo.getOnDnD && not new then model
        ...
        in  addASTxy (ASTxy (fixPos xy node.getBrickTab (List.map pickPos model2.getASTRoots)) (ASTne node bottom right)) model2
...
inputValuexy xy argCode value model =
    { model | getASTRoots = 
        ...
        |> List.map(\(ASTxy p (ASTne n b r)) -> 
            if p == xy && n.getBrickTab == model.nowTab then
...
viewASTxy model (ASTxy (x1, y1) (ASTne n b r)) = 
    ...
    in
    if not (model.nowTab == n.getBrickTab) then Keyed.node "div" [] []
    else Keyed.node "div"
